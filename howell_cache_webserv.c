...
int transfer_file(int src_fd, int dest_fd)
{
    ...
    while ((bytes_read = read(src_fd, buffer, BUFFER_SIZE)) > 0) {
        //printf("20get here: %p %s\n size:%ld bytes_read: %ld\n", buffer, buffer, strlen(buffer), bytes_read);
...
void handle_client_req_threaded(void* arg)
{
    ...
    if (stat(resource, &path_stat) == 0) {
        if (S_ISDIR(path_stat.st_mode)) {
            generate_dir_listing(resource, client_fd); // Generate and send directory listing
            goto jump;
        }
    } else {
        send_404(client_fd);
...
int check_cache(Cache* cache, int client_fd, char* content_type, char* resource, char* query, char* short_file_path){
    ...
    if(entry != NULL){
        //printf("6get here: %p %s\n size:%ld\n", entry->content, entry->content, sizeof(entry->content));
